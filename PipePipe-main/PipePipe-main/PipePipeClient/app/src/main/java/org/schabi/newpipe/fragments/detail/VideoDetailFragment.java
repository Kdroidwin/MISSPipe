...
public final class VideoDetailFragment
    ...
    private void openChannel(final String subChannelUrl, final String subChannelName) {
        try {
            if (currentInfo != null
                    && currentInfo.getServiceId() == ServiceList.MissAV.getServiceId()) {
                NavigationHelper.openSearchFragment(getFM(), currentInfo.getServiceId(),
                        toMissAvActressSearchQuery(subChannelName));
                return;
            }
    ...
    private static String toMissAvActressSearchQuery(final String uploaderName) {
        if (uploaderName == null) {
            return "";
        }
        return uploaderName.replace(" / ", " ").trim();
    }
    ...
    public boolean onLongClick(final View v) {
        ...
        if (id == R.id.detail_controls_background) {
            ...
            if (currentInfo.getServiceId() == ServiceList.MissAV.getServiceId()) {
                ShareUtils.copyToClipboard(requireContext(), currentInfo.getUploaderName());
                return true;
            }
            if (isEmpty(currentInfo.getSubChannelUrl())) {
                Log.w(TAG,
