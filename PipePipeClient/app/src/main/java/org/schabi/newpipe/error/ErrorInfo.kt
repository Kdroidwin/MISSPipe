...
class ErrorInfo(
    ...
    companion object {
        ...
        private fun getServiceNameFromId(serviceId: Int): String {
            val service = ServiceList.all().firstOrNull { it.serviceId == serviceId }
            val serviceName = try {
                NewPipe.getNameOfService(serviceId)
            } catch (_: Exception) {
                service?.serviceInfo?.name ?: if (serviceId == 7) "MissAV" else SERVICE_NONE
            }

            return serviceName + if (serviceId != -1) {
                " (" + (if (service?.hasTokens() == true) "Logged in" else "Anonymous") + ")"
            } else {
                ""
            }
        }
