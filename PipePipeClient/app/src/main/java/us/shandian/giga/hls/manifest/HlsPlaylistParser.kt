...
class HlsPlaylistParser {
    ...
    private fun resolveUrl(sourceUrl: String, reference: String): String {
        ...
        val javNoniMarked = sourceUrl.contains(JAVNONI_MARKER)
        val javSbMarked = sourceUrl.contains(JAVSB_MARKER)
        val cleanSourceUrl = sourceUrl.substringBefore(MISSAV_MARKER)
        ...
        } else if (javNoniMarked && !resolved.contains(JAVNONI_MARKER)) {
            resolved.substringBefore('#') + JAVNONI_MARKER
        } else if (javSbMarked && !resolved.contains(JAVSB_MARKER)) {
            resolved.substringBefore('#') + JAVSB_MARKER
        } else {
            resolved
        }
    }
    ...
    private companion object {
        ...
        const val JAVNONI_MARKER = "#javnoni=1"
        const val JAVSB_MARKER = "#javsb=1"
