...
public class DownloadMission extends Mission {
    ...
    private static final String JAVNONI_MARKER = "#javnoni=1";
    private static final String JAVSB_MARKER = "#javsb=1";
    ...
    HttpURLConnection openConnection(String url, boolean headRequest, long rangeStart, long rangeEnd) throws IOException {
        ...
        final boolean useJavNoniHeaders = url.contains(JAVNONI_MARKER) || isJavNoniUrl(url);
        final boolean useJavSbHeaders = url.contains(JAVSB_MARKER) || isJavSbUrl(url);
        ...
        url = url.replace(JAVNONI_MARKER, "");
        url = url.replace(JAVSB_MARKER, "");
        ...
        if (useJavNoniHeaders) {
            setJavNoniRequestProperties(conn);
        }
        if (useJavSbHeaders) {
            setJavSbRequestProperties(conn);
        }
    ...
    private static boolean isJavNoniUrl(final String url) {
        return url != null && (url.contains("jav-noni.vip/")
                || url.contains("luluvdo.com/")
                || url.contains("lulucdn.com/")
                || url.contains("tnmr.org/"));
    }

    private static boolean isJavSbUrl(final String url) {
        return url != null && (url.contains("jav.sb/") || url.contains(".18av.mov/"));
    }
    ...
    private static void setJavNoniRequestProperties(final HttpURLConnection conn) {
        conn.setRequestProperty("User-Agent", JAVNONI_USER_AGENT);
        conn.setRequestProperty("Referer", "https://luluvdo.com/");
        conn.setRequestProperty("Origin", "https://luluvdo.com");
        conn.setRequestProperty("Accept-Language", "ja,en-US;q=0.8,en;q=0.6");
    }

    private static void setJavSbRequestProperties(final HttpURLConnection conn) {
        conn.setRequestProperty("User-Agent", JAVSB_USER_AGENT);
        conn.setRequestProperty("Referer", "https://jav.sb/");
        conn.setRequestProperty("Origin", "https://jav.sb");
        conn.setRequestProperty("Accept-Language", "ja,en-US;q=0.8,en;q=0.6");
    }
