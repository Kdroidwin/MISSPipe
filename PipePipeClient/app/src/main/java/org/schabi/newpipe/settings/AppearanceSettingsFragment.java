...
import org.schabi.newpipe.util.AppIconHelper;
...
public class AppearanceSettingsFragment extends BasePreferenceFragment {
    ...
    public void onCreatePreferences(final Bundle savedInstanceState, final String rootKey) {
        ...
        final String appIconKey = getString(R.string.app_icon_key);
        final Preference appIconPreference = findPreference(appIconKey);
        if (appIconPreference != null) {
            appIconPreference.setOnPreferenceChangeListener((preference, newValue) -> {
                final String iconValue = AppIconHelper.sanitizeValue(newValue.toString());
                if (getContext() != null && AppIconHelper.applyIcon(getContext(), iconValue)) {
                    defaultPreferences.edit().putString(appIconKey, iconValue).apply();
                    return true;
                }
                return false;
            });
